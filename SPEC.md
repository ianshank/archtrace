...
| `architecture.docx` | Minimal OOXML via `zipfile` — **narrative + tables, no diagrams.** See below. |
| `jira-delta.json` | Model diff → ticket payloads |

**The `.docx` has no diagrams, and the spec says so out loud.** [Certain] Embedding one needs a raster or vector image: Mermaid→SVG needs Node, draw.io→PNG needs the drawio CLI, both excluded by §0.1. So `architecture.docx` is the narrative and the traceability matrix; diagrams ship as `.svg`/`.drawio`/Lucid. The named next increment is emitting **native DrawingML** shapes (`w:drawing` → `wps:wsp` boxes and connectors) straight into `document.xml` — pure stdlib, editable in Word, ~200 lines, and it is the single highest-value renderer left on the table because it makes the Word doc a deliverable instead of an appendix.
