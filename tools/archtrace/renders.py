...
def _docx(eng: Engagement) -> bytes:
    """Minimal, valid OOXML assembled with zipfile.

    Narrative and tables only. Embedding a diagram needs a raster or vector
    image, which needs Node (Mermaid) or the drawio CLI — both excluded. The
    named next increment is native DrawingML shapes emitted straight into
    document.xml from the same layout coordinates the SVG uses.
    """
